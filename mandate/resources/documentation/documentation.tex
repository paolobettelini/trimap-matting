...
\newglossaryentry{FFI}
{
    ...
    A mechanism that enables different programming
    languages to communicate and share data}
}
...
\newglossaryentry{trimap}
{
    name=trimap,
    description={A type of image segmentation (mask) where an image
    is divided into three regions: foreground, background, and unknown}
}

\newglossaryentry{matte}
{
    name=matte,
    description={A layer or mask that blocks out part of an image to
    create special effects or to combine multiple images}
}

\newglossaryentry{matting}
{
    name=matting,
    description={Process of separating the foreground
    and background of an image or video}
}
...
{
    name=JPG,
    description={Joint Photographic Experts Group. An image format
...
{
    name=PNG,
    description={Portable Network Graphics. An image format
    with lossless compression and transparency}
}
...
\newglossaryentry{WebP}
{
    ...
    A modern image format that provides superior
...
\newglossaryentry{segfault}
{
    name=segfault,
    description={An error in computer programming that
    occurs when a program attempts to access a memory
...
\requirement{00}{\gls{CLI} tool}{1}{1.1}{none}{
    A \gls{CLI} tool to execute background removal must be developed.
}{
    \subreq{00}{0}{The target image must be specified.}
    \subreq{00}{1}{The \gls{trimap} image can be specified.}
    \subreq{00}{2}{The soft mask can be specified.}
    \subreq{00}{3}{Either the soft mask or the \gls{trimap} must be specified.}
...
\requirement{01}{Image formats}{1}{1.0}{none}{
    ...
    \subreq{00}{0}{The \gls{JPG} format must be supported.}
    \subreq{00}{1}{The \gls{PNG} format must be supported.}
    \subreq{00}{2}{The \gls{WebP} format must be supported.}
}

\requirement{02}{Size check}{1}{1.0}{none}{
    The executable must assert that the target image and \gls{trimap} are of the same size.
}{%
}
...
The following diagrams illustrates the actions that an actor can perform using
the \gls{CLI} or \gls{GUI} applications.

\begin{tikzpicture}
    \begin{umlsystem}[x=1] {} % empty title
        \umlusecase[x=1, y=-2, name=br, width=2cm] {Background Removal}

        \umlusecase[x=7, y=-1, name=tu, width=2cm] {Target Upload}
        
        \umlusecase[x=7, y=1, name=im1, width=2cm] {Image Controls}
        \umlusecase[x=7, y=-3, name=t, width=2cm] {Alpha Mask Processing}

        \umlusecase[x=12, y=1, name=im3, width=1.5cm] {Image Controls}

        \umlusecase[x=12, y=-1.5, name=u, width=1.5cm] {Upload}
        \umlusecase[x=12, y=-3, name=d, width=1.5cm] {Draw Trimap}
        \umlusecase[x=12, y=-4.5, name=dl1, width=1.5cm] {Mask Download}

        \umlusecase[x=1, y=-6, name=brep, width=2cm] {Background Replacement}

        \umlusecase[x=7, y=-4.5, name=trans, width=2cm] {Transparent Removal}
        \umlusecase[x=7, y=-6, name=fill, width=2cm] {Fillage Removal}
        \umlusecase[x=7, y=-7.5, name=replace, width=2cm] {Replacement Removal}

        \umlusecase[x=12, y=-7.5, name=ru, width=2cm] {Replacement Upload}
    
        \umlusecase[x=12, y=-9.5, name=im2, width=2cm] {Image Controls}
        \umlusecase[x=1, y=-9.5, name=dl2, width=2cm] {Result Download}
    \end{umlsystem}
    \node [above] at (current bounding box.north) {Application};

    \umlactor[x=-1, y=0] {Actor}

    \umlassoc{Actor}{br}

    \draw [tikzuml dependency style] (br) -- node[above] {\(\ll includes \gg\)} (tu);
    \draw [tikzuml dependency style] (tu) -- node[left] {\(\ll includes \gg\)} (im1);
    \draw [tikzuml dependency style] (br) -- node[above] {\(\ll includes \gg\)} (t);
    \draw [tikzuml dependency style] (brep) -- node[right] {\(\ll extends \gg\)} (br);
    \draw [tikzuml dependency style] (replace) -- node[above] {\(\ll includes \gg\)} (ru);
    
    \draw [tikzuml dependency style] (dl1) -- node[above] {\(\ll extends\gg\)} (t);
    \draw [tikzuml dependency style] (d) -- node[above] {\(\ll extends\gg\)} (t);
    \draw [tikzuml dependency style] (u) -- node[above] {\(\ll extends\gg\)} (t);

    \draw [tikzuml dependency style] (u) -- node[left] {\(\ll includes\gg\)} (im3);
    
    \draw [tikzuml dependency style] (trans) -- node[above] {\(\ll extends\gg\)} (brep);
    \draw [tikzuml dependency style] (fill) -- node[above] {\(\ll extends\gg\)} (brep);
    \draw [tikzuml dependency style] (replace) -- node[above] {\(\ll extends\gg\)} (brep);
    \draw [tikzuml dependency style] (dl2) -- node[right] {\(\ll extends\gg\)} (brep);

    
    \draw [tikzuml dependency style] (ru) -- node[right] {\(\ll includes\gg\)} (im2);
\end{tikzpicture}
...
\Gls{matting} is a technique used to extract an object
from an image.
\Gls{trimap} \gls{matting} is a term used to refer to the process
of generating alpha \gls{matte}s\cite{matte} for an object
...
The initial plan was to dedicate approximately half the time to the \gls{CLI}
program and the remaining half to the \gls{GUI} application.
...
The project was more easy than I had anticipated. In fact,
I had built a working prototype of the \gls{CLI} program in less than 3 weeks.
The difficulty of the core functionality was overestimated,
and the core of the project was already built halfway through the project.
However, I found myself constantly adding little features and fixed
to the \gls{GUI} and \gls{CLI} throughout the second half of the working sessions.
I should have focused earlier on technical points and completing the documentation.
...
\makecell[tl]{
    ...
    (e.g. \texttt{.jpg} \(\rightarrow\) \textsc{\gls{JPG}})
}
}

\test{09}{Transparency and \gls{JPG} format}{None}{
    \item 
    \makecell[tl]{
        Execute the following command: \\
        \moveabit{\begin{lstlisting}[style=mattingcli]^^J
...
\makecell[tl]{
    A message to the \textsc{STDOUT} must warn about the usage of the \gls{JPG} format \\
...
\makecell[tl]{
    Each of the \gls{matting} commands must print en error indicating that the sizes \\
...
\test{13}{\Gls{trimap} mask generation}{Req-03}{
    \item Upload the target image to the website
    \item Draw the \gls{trimap} with the given tools
    \item Press the generate mask button
}{%
    A loading animation must appear on the website.
    After some time the corresponding alpha \gls{matte} mask must appear, replacing the loading animation.
}
...
\test{20}{Core dump}{Req-03}{
    \item Draw the \gls{trimap} such that it all black
    \item Press the generate mask button
}{%
...
The final product works as expected and it can be effectively used
for actual background removal.
I am not quite satisfied with the OpenCV library.
Their matting module causes core dumps if it cannot generate
an alpha matte mask. This has been quite obnoxious
as I could not develop the application precisely as I would have liked
it to be. Nevertheless, I really enjoyed being able to
have used the Rust programming language, which has simplified
my work a lot.
